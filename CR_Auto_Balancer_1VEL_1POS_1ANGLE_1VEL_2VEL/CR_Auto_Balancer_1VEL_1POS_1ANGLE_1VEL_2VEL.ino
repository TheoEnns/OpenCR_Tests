...
//              0.015, 0.010, 0.001,
              0.0, 0.0, 0.0,
              DIRECT, -MAX_SAFE_ANGLE, MAX_SAFE_ANGLE);
PID pidTrans2Vel(&actualTrans, &robotVel, &targetTrans,
              5.0, 5.0, 0.1,
              DIRECT, -150, 150);
PID pidAngle2Vel(&imu_angle, &balanceVel, &targetAngle,
//              18.000, 4.500, 0.30,
              10.000, 10.000, 0.100,
              DIRECT, -600, 600);
...
void loop() {
  ...
  while( true ){    
    ...
    imu_angle = IMU.rpy[1]; // (IMU.rpy[1]*1.0 + 1.0)/2.0;
    t_now = micros();
...
//    if(fabs(targetTrans - actualTrans) < 5.0) // Eliminate Small Trans Error
//      actualTrans = targetTrans;   
    pidTrans2Angle.Compute(delta);
   
    // Update pidAngle2Veland pidTrans2Vel  
//    if(fabs(targetAngle - imu_angle) < 0.5) // Eliminate Small Angle Error
//      imu_angle = targetAngle;
    ...
    motorVel_R = constrain(balanceVel + robotVel - rotation, -MAX_WHEEL_SPEED,MAX_WHEEL_SPEED);
    motorVel_L = constrain(balanceVel + robotVel + rotation, -MAX_WHEEL_SPEED,MAX_WHEEL_SPEED);
